...
use standard_query::services::{
    cma_api, local_matcher::LocalFileMatcher, samr_status, standard_parser,
};
...
fn extract_name_from_text(text: &str, code: &str) -> String {
    ...
    let std_re = regex::Regex::new(
        r"([A-Za-z]+[/]?[A-Za-z]*)\s*([0-9]+(?:[.\-][0-9]+)*)\s*[-\u{FF0D}\u{2014}]\s*([0-9]{4})",
    )
    .unwrap();
    let trailing_re = regex::Regex::new(r"[（(][^）)]*[）)]\s*$").unwrap();
...
async fn main() {
    ...
    for (i, (raw_code, input_name)) in input.iter().enumerate() {
        ...
        println!(
            "  SAMR: {}",
            validity.lines.first().map_or("", |l| l.text.as_str())
        );
        ...
        println!(
            "  CMA库: {}",
            cma_api_result.message.lines().next().unwrap_or("")
        );
...
fn write_output(rows: &[(String, String, String, String, String, String)]) {
    ...
    for (r, row) in rows.iter().enumerate() {
        for (c, val) in [&row.0, &row.1, &row.2, &row.3, &row.4, &row.5]
            .iter()
            .enumerate()
        {
