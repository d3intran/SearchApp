...
static NOISE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[A-Za-z]*\d{6,}").unwrap());
...
fn extract_page<R: Resolve>(
    ...
    for op in &ops {
        match op {
            ...
            Op::Restore => {
                ctm = ctm_stack.pop().unwrap_or_default();
            }
            Op::Transform { matrix } => {
                ...
                for item in array {
                    match item {
                        ...
                        TextDrawAdjusted::Spacing(o) => {
                            ...
                            text_matrix =
                                mat_mul(&text_matrix, &translate(adj / cur_size.max(1.0), 0.0));
                        }
                    }
                }
...
fn cluster(values: &mut [f32], tol: f32) -> Vec<f32> {
    if values.is_empty() {
        return vec![];
    }
    ...
    for &v in values.iter() {
        if let Some(last) = clusters.last_mut() {
            if (v - *last.last().unwrap_or(&0.0)).abs() <= tol {
                last.push(v);
            } else {
                clusters.push(vec![v]);
            }
...
pub fn parse(path: &str) -> AppResult<Vec<StandardEntry>> {
    ...
    for (page_idx, page_res) in file.pages().enumerate() {
        ...
        let Ok(page) = page_res else {
            continue;
        };
        let Ok(data) = extract_page(&page, &resolver) else {
            continue;
        };

        if let Some(grid) = detect_grid(&data) {
            // table mode: concatenate each column and extract standards.
            for ci in 0..grid.cols.len().saturating_sub(1) {
                ...
                for m in STD_RE.captures_iter(&text) {
                    let Some(m0) = m.get(0) else { continue };
                    let mstart = m0.start();
                    let mend = m0.end();
